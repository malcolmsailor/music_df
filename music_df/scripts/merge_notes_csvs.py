...
class Config:
    ...
    store_unmerged_indices: bool = False
...
def parse_args():
    ...
    parser.add_argument("--store-unmerged-indices", action="store_true")
...
def do_csv_file(inputf: str, config: Config, output_list: list | ListProxy):
    ...
    merged_df = merge_notes(
        music_df, store_unmerged_indices=config.store_unmerged_indices
    )
